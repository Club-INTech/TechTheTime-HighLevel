...
class MinimalSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        object_in_game_area = discrimination(msg,obj_groupes)
        self.get_logger().info('==== Begin =====')
        for k in range(len(object_in_game_area)):
            self.get_logger().info('I see : x = %d et y = %d' % (object_in_game_area[k][0],object_in_game_area[k][1]))
